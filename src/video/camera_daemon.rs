...
impl CameraDaemon {
    ...
    pub async fn start(&mut self) -> Result<(), DaemonError> {
        ...
        // Create frame channel (buffer up to 10 frames)
        let (tx, rx) = mpsc::channel::<FrameData>(10);
        ...
        tokio::spawn(async move {
            if let Err(e) = Self::processing_task(config, running, rx, stats).await {
