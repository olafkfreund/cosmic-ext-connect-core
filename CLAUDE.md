...
`cosmic-connect-core` is a pure Rust implementation of the KDE Connect protocol v7, designed as a cross-platform shared library via FFI (Foreign Function Interface). This library powers both:
...
cargo test --package cosmic-connect-core --lib network::discovery::tests
...
5. **FFI Layer** (`src/ffi/`)
   ...
   - UDL file: `src/cosmic_connect_core.udl` defines the FFI boundary
...
2. Reads `src/cosmic_connect_core.udl`
...
6. If needed, add FFI functions to `src/ffi/mod.rs` and `src/cosmic_connect_core.udl`
